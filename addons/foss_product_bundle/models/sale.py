...
class child_product_line(models.Model):
    ...
    # 新增的變數
    product_price = fields.Float(string='金額')
...
class SaleOrder(models.Model):
    ...
    def update_order_line(self,pack,amount=1):
        final_list = []
        for line in pack.child_line:
            if line.product_qty > 0:
                final_list.append((0, 0, {
                    'sequence': line.product_id.seq,
                    'product_id': line.product_id.id,
                    'product_uom_qty': line.product_qty * amount,
                    'price_unit':line.product_price,
                    'is_offer_product': True
                }))

        self.order_line = final_list

    def show_pack_choice_wizard(self):
        view = self.env.ref('foss_product_bundle.pack_choice_wizard')
        wiz = self.env['pack.choice'].create({'order_id': self.id})
        return {
            'name': '選取Bundle',
            'type': 'ir.actions.act_window',
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'pack.choice',
            'view_id': view.id,
            'target': 'new',
            'res_id': wiz.id,
